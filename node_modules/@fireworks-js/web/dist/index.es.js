...
const m = (e) => {
  ...
  "constructor",
  "prototype"
], y = (...e) => e.reduce((t, i) => (Object.keys(i).forEach((s) => {
  v.includes(s) || (Array.isArray(t[s]) && Array.isArray(i[s]) ? t[s] = i[s] : m(t[s]) && m(i[s]) ? t[s] = y(t[s], i[s]) : t[s] = i[s]);
}), t), {});
function b(e, t) {
  let i;
  return (...s) => {
...
class k {
  ...
  constructor({
    ...
    for (this.x = t, this.y = i, this.ctx = s, this.hue = n, this.gravity = r, this.friction = a, this.flickering = p, this.lineWidth = d, this.explosionLength = l, this.angle = c(0, Math.PI * 2), this.speed = o(1, 10), this.brightness = o(u.min, u.max), this.decay = c(h.min, h.max); this.explosionLength--; )
...
class M {
  ...
  constructor({
    ...
    for (this.x = t, this.y = i, this.sx = t, this.sy = i, this.dx = s, this.dy = n, this.ctx = h, this.hue = r, this.speed = a, this.traceLength = u, this.acceleration = p, this.totalDistance = f(t, i, s, n), this.angle = Math.atan2(n - i, s - t), this.brightness = o(50, 70); this.traceLength--; )
      this.coordinates.push([t, i]);
  }
  update(t) {
...
class z {
  ...
  raf;
  constructor(t, i = {}) {
    this.target = t, this.container = t, this.opts = new C(), this.createCanvas(this.target), this.updateOptions(i), this.sound = new L(this.opts), this.resize = new O(
      this.opts,
      this.updateSize.bind(this),
      this.container
    ), this.mouse = new S(this.opts, this.canvas), this.raf = new E(this.opts, this.render.bind(this));
  }
  get isRunning() {
    return this.running;
  }
  ...
  createTrace() {
    ...
    this.traces.push(
      new M({
        ...
        y: this.height,
        dx: this.mouse.x && a.move || this.mouse.active ? this.mouse.x : o(s.x, s.width - s.x * 2),
        dy: this.mouse.y && a.move || this.mouse.active ? this.mouse.y : o(s.y, s.height * 0.5),
        ctx: this.ctx,
        hue: o(t.min, t.max),
        speed: h,
        acceleration: r,
...
class T extends HTMLElement {
  ...
  constructor() {
    super();
    const t = this.attachShadow({ mode: "open" }), i = document.createElement("div"), s = document.createElement("style");
    t.append(s, i), this.attributeChangedCallback();
  }
  connectedCallback() {
    if (!this.isConnected) return;
    const t = this.shadowRoot.querySelector("div");
    !this.fireworks && t && (this.fireworks = new z(t, this.options), this.fireworks.start());
  }
  ...
  attributeChangedCallback() {
    ...
    if (t)
      try {
        this.options = JSON.parse(t), this.fireworks?.updateOptions(this.options);
      } catch (s) {
        console.error("Attribute `options` failed parsed:", s);
      }
    const i = this.getAttribute("style");
    i && (this.shadowRoot.querySelector("style").textContent = `div { ${i} }`);
  }
}
customElements.define("fireworks-js", T);
export {
  T as Fireworks,
  T as default
};
